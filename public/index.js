...
function processAllInputs(){
  var rawInput = input.value;
    ...
    $('#cleanedInputDat').html('<code>'+cleanedInput+'</code><br>(' + cleanedInput.length + ' digits long)');
    ...
    $('#qrcode').empty(); $('#qrcode').qrcode(convertedToBase); // See "https://codepen.io/hchiam/pen/BeMQZe" for the working inspiration of this.
    $('#qrFooter').html('<kbd>'+convertedToBase+'</kbd>');
    var fullSerial = $('#asmblyLocCode').val() + '-' + convertToBase(cleanedInput,36);
    $('#qrcodeFullSerial').empty(); $('#qrcodeFullSerial').qrcode(fullSerial);
    $('#qrFooterFullSerial').html('<kbd>' + fullSerial + '</kbd>');
};
...
function toPilotSpk(text) {
  var text = new String(text); //not really necessary, just a precaution.
